...
def make_a(regres, x_train):
    ...
    for sample in x_train:
        a = np.dot(regres, sample) 
#         a = a.reshape(1, channels, freq, time)
        a_list.append(a[0])
...
#     a_list = []
#     for sample in x_train:
#         channels, freq, time = sample.shape
#         a = np.dot(regres, np.transpose(sample, [1, 0, 2]))[0]
#         a = a.reshape(-1)
#         a_list.append(a)
#     return np.array(a_list)
...
#     b_list = []
#     for sample in x_train:
#         b = np.dot(regres.T, sample)[0].reshape(-1)
#         b_list.append(b)
#     return np.array(b_list)
...
def simple_linear_regression(alphas, repr_train, repr_val, y_train, y_val, zone, normalize):
    ...
    for index, alpha in enumerate(alphas):
        ridgereg_a = Ridge(alpha, fit_intercept=False, normalize=normalize)
        ridgereg_a.fit(repr_train, y_train[:, zone])
        
        predicted_y_test_a = ridgereg_a.predict(repr_val)
        
#         print(predicted_y_test_a.shape, repr_val.shape, y_val[:, zone].shape)
    ...
    def fit(self, x_train, x_val, y_train, y_val):
        ...
        for zone in range(self.len_zones):
            ...
            while cos_regr1 > self.epsilon and cos_regr2 > self.epsilon:
                ...
                # learn freq-time components
                # apply some topological weight ( 1, n_channels)
                spect_repr_train = make_a(topo_regr0, x_train)
                spect_repr_val = make_a(topo_regr0, x_val)
                
                
                a_coefs, _ = simple_linear_regression(self.alphas, spect_repr_train, spect_repr_val, y_train,
                                                      y_val,zone, self.normalize)
                print('Size step a: ', spect_repr_train.shape, a_coefs.shape)
                
                spect_regr1 = a_coefs.reshape(len_spect, 1)
                cos_regr1 = n_cos(spect_regr0, spect_regr1)
                
                
                # learn space component
                # apply some topological weight (time8freq, 1)
                topo_repr_train = make_b(spect_regr1, x_train)
                topo_repr_val = make_b(spect_regr1, x_val)
                
                
                b_coefs, model = simple_linear_regression(self.alphas, topo_repr_train, topo_repr_val,
                                                          y_train, y_val, zone, self.normalize)
                print('Size step b: ', topo_repr_train.shape, b_coefs.shape)
        ...
        self.model = models
        ###  Remove because fucntion don't work :(
#         self.patterns_spect_ = patterns_evaluation(topo_coefs, y_val.shape[-1], x_train, x_val)
#         self.patterns_topo_ = patterns_evaluation(spect_coefs, y_val.shape[-1], x_train, x_val)
        return self
